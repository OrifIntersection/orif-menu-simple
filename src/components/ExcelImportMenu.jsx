...
function ExcelImportMenu({ onImport }) {
  const [week, setWeek] = useState("");      // ex: "2025-49"
  ...
  function handleFileChange(event) {
    ...
    if (!week) {
      alert("Merci d'indiquer le numéro de semaine (ex: 2025-49) avant d'importer.");
    ...
    reader.onload = (e) => {
      try {
        ...
        // Trouver les index des colonnes (on n'utilise pas semaineIdx car on force la semaine saisie)
        ...
        const menus = dataRows
          .filter((row) => row[jourIdx] && row[momentIdx] && row[platIdx]) // garde seulement les lignes avec Jour, Moment, Plat
          .map((row) => ({
            semaine: week,     // on force la semaine à ce que l'utilisateur a choisi
        ...
        console.log(`✅ ${menus.length} plats importés depuis Excel`);
