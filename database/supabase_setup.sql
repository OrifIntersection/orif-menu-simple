...
CREATE TABLE public.menu_items (
  ...
  dish_id bigint NOT NULL,
  PRIMARY KEY (id),
  UNIQUE (menu_day_id, meal_type_id, dish_id),
...
CREATE TABLE public.meal_items (
  ...
  CONSTRAINT unique_meal_assignment UNIQUE (date, meal_type_id, dish_id)
);

-- =====================================================
-- ETAPE 2 : INDEX UTILES
-- =====================================================
-- plus d'index sur category_id
...
-- LUNDI MIDI
INSERT INTO public.menu_items (menu_day_id, meal_type_id, dish_id)
SELECT md.id, mt.id, d.id
FROM public.menu_days md, public.meal_types mt, public.dishes d
WHERE md.day_name='Lundi' AND md.day_date='2025-11-04' AND mt.code='MIDI' AND (
  d.name IN ('Salade verte','Poulet roti','Pates','Haricots verts','Yaourt')
);
...
WHERE md.day_name='Lundi' AND md.day_date='2025-11-04' AND mt.code='SOIR' AND (
  d.name IN ('Carottes rapees','Steak hache','Riz','Courgettes','Compote')
);
...
WHERE md.day_name='Mardi' AND md.day_date='2025-11-05' AND mt.code='MIDI' AND (
  d.name IN ('Salade Cesar','Poisson pane','Pommes de terre','Brocolis','Fruit')
);
...
WHERE md.day_name='Mardi' AND md.day_date='2025-11-05' AND mt.code='SOIR' AND (
  d.name IN ('Taboule','Saucisse de Strasbourg','Puree','Carottes','Mousse au chocolat')
);
...
WHERE md.day_name='Mercredi' AND md.day_date='2025-11-06' AND mt.code='MIDI' AND (
  d.name IN ('Betteraves','Escalope de dinde','Frites','Ratatouille','Tarte aux pommes')
);
...
WHERE md.day_name='Mercredi' AND md.day_date='2025-11-06' AND mt.code='SOIR' AND (
  d.name IN ('Salade verte','Poulet roti','Riz','Haricots verts','Yaourt')
);
...
WHERE md.day_name='Jeudi' AND md.day_date='2025-11-07' AND mt.code='MIDI' AND (
  d.name IN ('Carottes rapees','Steak hache','Pates','Courgettes','Compote')
);
...
WHERE md.day_name='Vendredi' AND md.day_date='2025-11-08' AND mt.code='MIDI' AND (
  d.name IN ('Taboule','Escalope de dinde','Puree','Carottes','Mousse au chocolat')
);
...
WHERE md.day_name='Vendredi' AND md.day_date='2025-11-08' AND mt.code='SOIR' AND (
  d.name IN ('Betteraves','Saucisse de Strasbourg','Frites','Ratatouille','Tarte aux pommes')
);
...
INSERT INTO public.meal_items (date, meal_type_id, dish_id)
SELECT '2025-11-12'::date, mt.id, d.id
FROM public.meal_types mt, public.dishes d
WHERE mt.code='MIDI' AND d.name IN ('Salade de tomates','Boeuf bourguignon','Puree de pommes de terre','Haricots verts','Creme brulee');

INSERT INTO public.meal_items (date, meal_type_id, dish_id)
SELECT '2025-11-13'::date, mt.id, d.id
FROM public.meal_types mt, public.dishes d
WHERE mt.code='SOIR' AND d.name IN ('Betteraves','Escalope de porc','Riz basmati','Courgettes','Yaourt aux fruits');
...
--- les anciens
-- =====================================================
-- NETTOYAGE COMPLET ET CRÉATION DE LA BASE DE DONNÉES
-- =====================================================
...
-- ÉTAPE 5: PLATS (25 plats : 5 par categorie)
-- Salades
INSERT INTO public.dishes (name, description) VALUES
  ('Salade verte', 'Salade verte fraiche de saison'),
  ('Salade Cesar', 'Salade romaine, croutons, parmesan, sauce Cesar'),
...
-- ÉTAPE 6: MENU SEMAINE 45/2025 (4-8 novembre)
INSERT INTO public.menus (year, week_number, week_label, start_date, end_date)
VALUES (2025, 45, '4 au 8 novembre 2025', '2025-11-04', '2025-11-08');
...
-- ÉTAPE 7: REMPLISSAGE MENU (50 items)
-- LUNDI MIDI
...
-- ÉTAPE 8: AUTHENTIFICATION ET SÉCURITÉ
-- Fonction pour créer automatiquement un profil lors de l'inscription
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (user_id, full_name, role)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'full_name', 'Nouvel utilisateur'), 'viewer');
  RETURN NEW;
END;
...
-- Créer le profil pour l'utilisateur déjà connecté
INSERT INTO public.profiles (user_id, full_name, role)
VALUES ('1ebb59cc-e034-4f09-b8a5-68e07015d11d', 'Admin ORIF', 'admin')
...
-- VÉRIFICATION FINALE
-- =====================================================
SELECT 'Profils créés:' as info, count(*) as total FROM public.profiles;
SELECT 'Menus créés:' as info, count(*) as total FROM public.menus;
SELECT 'Plats créés:' as info, count(*) as total FROM public.dishes;
SELECT 'Items menu créés:' as info, count(*) as total FROM public.menu_items;
